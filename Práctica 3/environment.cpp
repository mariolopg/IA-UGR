...
bool Environment::JuegoTerminado() const{
    ...
    for (int i=0; !terminado and i<7; i++)
      for (int j=0; !terminado and j<7; j++){
          if (maze_[i][j]!=0)
            terminado=EnLinea(i,j,ganador);
      }


    if (!terminado){
      bool listaAcciones[8];
      if (possible_actions(listaAcciones) == 0){
        return true;
      }
    }
...
int Environment::RevisarTablero() const{
    ...
    for (int i=0; !cuatro_en_raya and i<7; i++)
      ...
      /*if (cuatro_en_raya){
        PintaTablero();
        cerr << "Gana el jugador " << ganador << endl;
        char ch;
        cin >> ch;
      }*/
...
void Environment::AplicarExplosion(int fila, int columna){
    ...
    for (int x=0; x<7; x++){
        ...
        if (maze_[t][x]==adversario or maze_[t][x]==adversario+3 or maze_[t][x]==jugador_activo_+3){
