...
int GUI::startDraw(int argc,char **argv){
	...
	main_window=glutCreateWindow("PRACTICA 3: CONECTA 4 BOOM");
...
//		new GLUI_Column(score_glui,false);
      ...
      main_glui=GLUI_Master.create_glui_subwindow(main_window,GLUI_SUBWINDOW_RIGHT);
		...
		new GLUI_EditText(main_glui,"Movimientos:",display_step);
    ...
    if (partida_remota){
        if (esperando_server){
          if (!env->JuegoTerminado()){
              ...
              int esta_jugando = env->JugadorActivo();
              char aux;
              ...
              env->ChangePlayer();
              UpdateButton();
              if (esta_jugando != env->JugadorActivo()){
                UpdateButton();
	...
	switch(id){
      ...
      glutPostRedisplay();
        ...
        partida_remota=true;
			...
			else if (modo==4 or modo==5 or modo==6){ // Vs Ninjas
	      char mensaje[1024];

        if (modo ==4){
          Cliente.Establecer_Conexion(field_ip_ninja, "5127");
          cout << "\tJugando contra el NINJA 1 (muy asequible)\n";
        }
        else if (modo==5){
      ...
      esta_jugando = env->JugadorActivo();
			doNextMove();
      if (modo == 1){
        if (esta_jugando != env->JugadorActivo()){
          UpdateButton2();
    ...
    case CURSOR_BOOM_BTN_ID:
         ...
         switch (modo){
             ...
             case 1: // Vs mi heuristica
                     esta_jugando = env->JugadorActivo();
                     if ((esta_jugando==1 and jugador==0) or (esta_jugando==2 and jugador==1)){
                       do_move(id-501);
                       if (esta_jugando != env->JugadorActivo()){
                         UpdateButton2();
                         myGlutDisplay();
...
void GUI::do_move(int accion){
  ...
  env->ChangePlayer();
  if (env->JugadorActivo() != esta_jugando){
    UpdateButton();
  }
